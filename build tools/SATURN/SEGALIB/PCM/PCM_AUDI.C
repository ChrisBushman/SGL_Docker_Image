 ...
 * Module 	:PCM buffer management, PCM data copy（AIFF file）
 * File		:pcm_audi.c
 * Date		:1997-08-25
 * Version	:1.25
 * Author	:Y.T, Y.H
 *
 *--------------------------------------------------------------------------*
 * Update	:1994-09-29	1.00	Y.T	新規作成
 *			:1996-07-05	1.70	Y.H	SAP ファイル対応
 *			:1996-10-14	1.21	Y.H	SHC Ver. 3.0F対応
 *          :1997-08-25 1.25    N,T Stereo 16Bitバグ修正
 *
 ****************************************************************************/
...
/************************************************************************/
/*  ＣＰＵ－ＤＭＡ転送の起動   */
#define COPY_CPUDMA_PCM(st)													\
	DMA_CpuMemCopy4( (void *)(st)->copy_tbl[(st)->copy_idx].dst1,			\
					(void *)(st)->copy_tbl[(st)->copy_idx].src,				\
					(Uint32)((st)->copy_tbl[(st)->copy_idx].size) >> 2 )
...
【引　数】
	hn					(入力)　ハンドル
	addr_write1			(出力)　PCMバッファ書き込みアドレス(モノラル or ステレオ-R)
	addr_write2			(出力)　PCMバッファ書き込みアドレス(ステレオ-L)
...
void pcm_GetPcmWrite(PcmHn hn, Sint8 **addr_write1, Sint8 **addr_write2, Sint32 *size_write, Sint32 *size_write_total)
{
	...
	*size_write_total = st->pcm_bsize - 
	...
	if (*size_write_total == 0) {
		...
		*addr_write1 = para->pcm_addr + st->pcm_write_offset;	/*  Rch  */
		*addr_write2 = st->pcm2_addr  + st->pcm_write_offset;	/*  Lch  */
	}
...
STATIC void pcm_SetMixCopyTbl(PcmHn hn, Sint32 size_copy)
{
	...
	Sint32		size_2nd; 			/* 2回目のpcmロードサイズ(1チャンネル分) */
...
void	pcm_AudioMix(PcmHn hn)
{
	...
	/*  1996-10-14 Y.H  (start)  */
	/*  サイズが４倍数でない時、最後まで再生されないバグを修正  */
#if		0
	/*  1996.05.31 Y.H  (start)  */
	/*  いずれも４倍数であるようにした  */
#if		0
 ...
 *【NOTE】	1996.05.31 Y.H
 *
 *	関数 pcm_AudioProcess() は廃止。
 *
 *  関数 PCM_MeCreate() において、
 *		st->audio_process_fp = pcm_AudioMix;
 *  のように関数へのポインタを代入する。
 *  pcm_AudioMix は AIFF データ処理関数へのポインタ。
 *
 *  SAP データ処理の場合は、
...
void pcm_FillSilence(PcmHn hn)
{
	...
	Sint32		size_write, size_write_total;
	Sint32		size_2nd, 			/* 2回目のpcmロードサイズ(1チャンネル分) */
				size_mono;			/* オーディオサンプルの１チャンネル分のサイズ */
	Sint8  		*addr_write1, *addr_write2;
