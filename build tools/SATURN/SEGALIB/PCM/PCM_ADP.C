 ...
 *
 *--------------------------------------------------------------------------*
 * Update	:1994-10-14	1.00	Y.T	新規作成
...
void pcm_AudioAdpcmSct(PcmHn hn)
{
	...
	Sint8 		*addr_w1, *addr_w2;	/* PCMバッファ書き込みアドレス 			*/
	Sint32		size_w, size_w_t; 	/* PCMバッファ書き込み可能サイズ [byte] 	*/
	Sint32		ring_bsize, 		/* リングバッファの在庫サイズ [byte] 		*/
				ring_sg, 		/* リングバッファの在庫サウンドグループ数[sect/1ch]	*/
				pcm_sg, 		/* PCMバッファの受け入れサウンドグループ数[sect/1ch]*/
				copy_sg, 		/* 実行コピーサウンドグループ数 [sect/1ch] 		*/
...
/*	Sint32 		sample_now; */
	...
	/* ＰＣＭバッファが受け入れ可能なサイズを、サウンドグループ数に換算する。 */
...
void pcm_SetInfoAdpcm(PcmHn hn)
{
	...
	*/
	...
	PCM_INFO_CHANNEL(&st->info) = -1;			/* 先頭セクタのciを見て判断 */
	PCM_INFO_SAMPLING_RATE(&st->info) = -1;		/* 先頭セクタのciを見て判断 */
