 ...
 *          :1997-08-25 1.26    N.T Total Level対応( SND I/F Ver.1.31 対応)
 *
 ****************************************************************************/
	...
	for (i = 0; i < PCM_HN_MAX; i++) {
		...
		if (pcm_bittbl[para->pcm_stream_no] & pcm_play_number) {
			...
			if (pcm_cnt_vbl_in - st->vbl_pcm_start >= 3) {
				...
				/*  1996.06.05 Y.H  (start)  */
				PCMXLIB_STAT_XPLAY_ON(st) = 1;		/*  SAP 1st-Load 指示  */
				/*  1996.06.05 Y.H  ( end )  */
...
void PCM_DrvChangePcmPara(PcmHn hn, Sint32 level, Sint32 pan)
{
	...
	SND_PRM_TL(chg_prm)     = para->pcm_total_level;    /* tany 97/8/17 */

    imask = get_imask();
    set_imask(15);

	while (SND_ChgPcmTL(&chg_prm) == SND_RET_NSET) {   /* tany 97/8/17 */
		set_imask(imask);
		pcm_Wait(100);
		set_imask(15);
	}
...
void PCM_DrvStartPcm(PcmHn hn)
{
    ...
    set_imask(15);
	   ...
	   /* tany 97/8/17 */
	while (SND_StartPcmTL(&start_prm, &chg_prm) == SND_RET_NSET) {
		set_imask(imask);
		pcm_Wait(100);
		set_imask(15);
	}
