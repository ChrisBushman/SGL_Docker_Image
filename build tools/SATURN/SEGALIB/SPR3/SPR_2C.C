 ...
 *  Updated by H.O on 1996-02-16 Ver.1.00a1
 *                 SPR_2SetChar(),allocBlock(),SPR_2UsrClip()修正
 *  Updated by K.K on 1996-09-25         1byteカナコード殲滅作戦
 *  1997-05-15 A.H(SOJ)
 *       イレース用ポリゴンの Y座標設定が不定であるバグを修正。
 *       イレース用ポリゴンの描画を禁止出来る設定を追加。
 *
 ...
 *     (3) Uint16  color        - <i>  カラーデータ（カラーバンクorルックアップテーブルNo）
...
/* '96-02-09 H.O start */
	...
	if( colorMode >= COLOR_5 )
		...
		blockSize = charSize;
#else
/* charSize is number of dots. */
	if( colorMode >= COLOR_5 )
		charSize *= 2;
	else if( colorMode <= COLOR_1 )
		charSize /= 2;
/* charSize is now size of charactor data. */
	blockSize = charSize;
#endif
/* '96-02-09 H.O end */
	...
	*/
	...
	w = (Uint16*)xy;
        ...
        *w   = SpScreenY-1;
        ...
        spCmd->ax      = 0;
        spCmd->ay      = 0;
#ifdef USE_ERASE_POLIGON
		/* 1997-05-15 A.H(SOJ) ライブラリ内部でイレース用ポリゴンを描画するか
		 *                     どうかを選択可能な様に変更
		 */
        spCmd = SpCmdTbl + SpCmdNo++;
		...
		*/
...
#else
	#if	0
		/* 1997-05-15 A.H(SOJ) イレース用ポリゴンの Y座標設定が
		 *                     不定であるバグを修正 */
		spCmd->ax = 0;
		spCmd->ax = 0;
		...
		spCmd->by = 0;
		spCmd->cx = SpScreenX-1;
		spCmd->cy = SpScreenY-1;
		spCmd->dx = 0;
		spCmd->dy = SpScreenY-1;
	#endif
#endif
        spCmd->grshAddr = 0;
...
#if 0 /* 96-02-16 H.O Start */
/*
 *96-02-16 H.O
 *  高橋氏のコードに対するBUG修正
 *  引数のxyはXyInt型構造体要素2個の配列である。
 */
#if	0
	/*
	**■1995-08-28	高橋智延
...
#else
		p = ( Vaddr )( ( ( Uint32 )memblk + blocksize - ( Uint32 )VRAM ) >> 3 );
#endif
/* '96-02-09 H.O end */
