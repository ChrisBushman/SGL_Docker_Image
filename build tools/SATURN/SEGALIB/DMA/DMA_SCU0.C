 ...
 *  AUTHOR(S)
 *
 *      1997-06-09  A.H Ver.1.06
 *  MOD HISTORY:
 *
 *      1994-10-12  N.T Ver.1.04
 *      1996-09-25  K.K Ver.1.05 コメント内1byteカナコード殲滅作戦
 *      1997-06-09  A.H Ver.1.06 SEGA_SYS Ver2.50 (MACレジスタ待避)に対応
 *-----------------------------------------------------------------------------
 */
...
#define REG_DXMD    ((volatile Uint32 *)0x25fe0014)  /* DMAモード、アドレス更新、起動要因　*/
#else
...
#define REG_DXMD  ((volatile Uint32 *)0x06060014)  /* DMAモード、アドレス更新、起動要因　*/
#endif /* _DEB */
/******** チャネルオフセット *************************************************/
...
#ifndef DMA_VER_106  /* 1997-06-09 Ver.1.06 A.H(SOJ) SEGA_SYS Ver2.50 対応 */
...
#else
	/* MACレジスタ待避は、割込みルーチン内で対応 A.H(SOJ) */
	#define STOP_COM(ch, vector_num)\
		do{\
...
#ifndef DMA_VER_106  /* 1997-05-21 A.H(SOJ) SEGA_SYS Ver2.50 対応 */
    dma_scu_vector[ch] = INT_GetScuFunc(now_vector_num); /* ベクタ待避SCU */
    dma_vector[ch] = INT_GetFunc(now_vector_num);    /* ベクタ待避        */
    dma_int_status[ch] = DMA_SCU_MV;
    INT_SetScuFunc(now_vector_num, intr_func);       /* ベクタ登録        */
    INT_SetFunc(now_vector_num, NULL);         /* SCU割り込み関数の再登録 */
#else
/* MACレジスタ待避は、割込みルーチン内で対応 A.H(SOJ) */
    dma_scu_vector[ch] = SYS_GETUINT_NO_MACSAVE(now_vector_num); /* ベクタ待避SCU */
    dma_vector[ch] = SYS_GETSINT(now_vector_num);      /* ベクタ待避        */
    dma_int_status[ch] = DMA_SCU_MV;
    SYS_SETUINT_NO_MACSAVE(now_vector_num, intr_func); /* SCU割込みの登録 */
    SYS_SETSINT(now_vector_num, NULL);                 /* ベクタ登録   */
#endif
