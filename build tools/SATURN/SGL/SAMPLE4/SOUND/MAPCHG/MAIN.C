...
/*							*/
/*     Modified tany 97/7/10                            */

#include    "sl_def.h"

/*------------------------------------------------------------------------*/

#define     SoundBindBuf       	0x25a0b000 /* For map 0 Collect file   */
#define     SoundToneBuf       	0x25a0b000 /* For map 1 tone bank data */
#define     SoundSeqBuf		0x25a6f000 /* For map 1 sequence data  */
...
extern Uint8 sddrvstsk[];
extern Uint32 sddrvsize;

extern char   tone[];
extern Uint32 tone_size;
extern char   seq1[];
extern Uint32 seq1_size;
/*------------------------------------------------------------------------*/
...
void init_sound(){
    slInitSound(sddrvstsk , sddrvsize , (Uint8 *)sound_map , mapsize) ;
    slDMACopy(sound_dat , (void *)(SoundBindBuf) , datsize) ;
}
...
/*
   SetTrnsEnd
   Set sound data transfer end flag.
*/
void SetTrnsEnd( void ) {
  Uint8 *addr = ( Uint8 * )( 0x25a00500 );
...
void sound_test(EVENT *evptr){
    ...
    if ( padp & PER_DGT_TL ) {
      ...
      slSndFlush();
      map = 1;
      slWaitSound( slSndMapChange( map ) );
      slDMACopy( tone, ( void * )( SoundToneBuf ) ,tone_size );
      slDMACopy( seq1, ( void * )( SoundSeqBuf ) ,seq1_size );
      slDMAWait();
      SetTrnsEnd();
      slSndMixChange( 0, 0 );
    }
