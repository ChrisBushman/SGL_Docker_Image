...
static Uint32 *g_movie_buf = (Uint32 *)0x00200000;	/* DRAM ADDRESS */
...
static CpkHn createMovie(void)
{
  ...
  if (cpk == NULL) {
    slPrint( "Error while create Handle.", slLocate( 9, 3 ) );
    while( -1 ) slSynch();
  }
