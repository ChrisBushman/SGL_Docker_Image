/*                                                              */
/*    アプリケーション初期化 & 起動プログラムサンプル           */
/*                              V1.10  '94.11.11  S.U and T.S   */
/*                              V1.20  1997-07-04 A.H           */
/*                              V1.30  1997-08-29 A.H           */
/*                              V1.31  1997-08-29 A.H           */
/*                                                              */
/*    動作タイミングと機能について：                            */
...
#if 0
    変更履歴
       Ver.1.20  SYSライブラリ Ver.2.50 対応
                 割込みルーチン内部で、MACH,MACLレジスタを待避していない
                 バグを修正。
                 IPサイズは約 1350H (ワールドワイドの場合)
       Ver.1.30  初期化完了後のジャンプ先を、
                 sys_id.src の 1stREAD ADDRESを参照して
                 決定する様に改良。
                 APP_ENTRY の値を書き換える必要が無くなった。
                 (APP_ENTRY を定義すると、以前と同じ動作をする。)
       Ver.1.31  DATEL社製 未承認カートリッジのチェック機能を
                 追加(デフォルトはチェックしない)
                 チェックしたい場合は、USE_CHKOUT_DATEL_CART
                 を define する事。
                 この機能を使用すると、
                 DATEL社製 「UNIVERSAL ADAPTER」がカートリッジ
                 端子に接続中の場合は、ゲームの起動を中止し、
                 マルチプレイヤを起動する。
                 なお、IPサイズに注意する事。
			#define USE_CHK_DATEL_CART
#endif


#if 0  /* GNU環境では、実行バイナリのサイズが大きくなるのでコメントアウト */
#define USE_CHK_TRAYOPEN     /* IP.BIN 内で、トレイオープンチェックをする */
#include "sega_cdc.h"
#endif

#include "sega_sys.h"

/* 1997-08-29  Ver.1.30  by A.H(SOJ) */
         /* 1stREAD ADDRES とは別のアドレスにジャンプしたい場合は、*/
         /* APP_ENTRY を使用して下さい。                           */
#if 0
/* 先読みしたプログラムの実行開始アドレス（メイン処理完了後にジャンプ）*/
#define	APP_ENTRY	(0x6010000)
#else
/* sys_id.src 内の、1stREAD ADDRES を参照して、そのアドレスにジャンプ */
#define	APP_ENTRY_P	(0x60020F0)
#endif
...
#ifdef USE_CHK_DATEL_CART
static void chk_datel(void);     /* Ver.1.31 by A.H(SOJ) */
#endif
...
void main(void)
{
	/* 注意：AUTO 変数をとると、APP_ENTRY_P のプログラムに */
	...
	SYS_SETUINT_NO_MACSAVE(VBI_NUM, vbIrtn); /* ＶＢイン処理登録    */
	SYS_SETUINT_NO_MACSAVE(VBO_NUM, vbOrtn); /* ＶＢアウト  〃      */
	...
	SYS_SETUINT_NO_MACSAVE(VBI_NUM, (void(*)())0 ); /* フック       */
	SYS_SETUINT_NO_MACSAVE(VBO_NUM, (void(*)())0 ); /*   再初期化   */

#ifdef USE_CHK_TRAYOPEN
	if (CDC_GetHirqReq() & CDC_HIRQ_DCHG ) SYS_EXECDMP();
#endif
#ifdef USE_CHK_DATEL_CART
	chk_datel();     /* Ver.1.31 by A.H(SOJ) */
#endif
#ifdef APP_ENTRY
	((void(*)())APP_ENTRY)();    /* 次の実行開始アドレス */
#else
  /* sys_id.src 内の、1stREAD ADDRES を参照して、そのアドレスにジャンプ */
	(*((void(**)(void))APP_ENTRY_P))();    /* 次の実行開始アドレス */
#endif
}
...
{
	Uint32 macsave[2];  /* Ver1.20 割込みルーチン内 MACH,MACLレジスタ待避 */

	SYS_GetMac(macsave);  /* Ver1.20 */
	vbIcnt++;
	SYS_SetMac(macsave);  /* Ver1.20 */
}
...
/* Ver.1.31 by A.H(SOJ) */
#ifdef USE_CHK_DATEL_CART
#define CHK_DATEL_BYTES  (23)
           /*                            12345678901234567890123 */
static char *chk_datel_string = (char *)"SEGA SEGASATURN  DATEL ";
static volatile char	*chk_addr = (char *)0x22000000;
#endif
...
static void chk_datel(void)     /* Ver.1.31 by A.H(SOJ) */
{
	...
	for(i = 0; i < CHK_DATEL_BYTES; i++ ){
	    if( *chk_addr++ != *chk_datel_string++ )
	         return;  /* OK */
	}
	/*  NG  */
	SYS_EXECDMP();   /* マルチプレイヤへジャンプ */
}
#endif
