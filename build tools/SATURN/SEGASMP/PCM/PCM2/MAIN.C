/******************************************************************************
 *	ソフトウェアライブラリ
 *
 *	Copyright (c) 1994,1995 SEGA
 *
 * Library	:ＰＣＭ・ＡＤＰＣＭ再生ライブラリ
 * Module 	:sample program of STREAM PLAY MODE
 * File		:smppcm2.c
 * Date		:1995-03-31
 * Version	:1.16
 * Auther	:Y.T
 *
 * Comment	:
 *	ストリームシステムを使ってＣＤ上のファイルを再生するサンプル
 *	ファイル名は "SAMPLE.AIF"
 *
 ****************************************************************************/

/*------------------------- プログラム選択 -------------------------*/

/* VTV, MON を使用したデバッグの時に定義する。 */
/* #define SMPPCM_DEBUG */

/*------------------------- インクルード -------------------------*/
#include "inc.h"

/* 再生するファイル名 */
#if 0
Sint8 filename[] = "SAMPLE.AIF";
#else
Sint8 filename[] = "KOE0001.AIF";
#endif

/* -----------------------  error status -------------------------*/
#if 0
Sint16 *err_status = (Sint16 *)0x260f0000;
SndRet	*snd_error = (SndRet *)0x260f7000;
#endif

int	SclInit = 0, PcmInit = 0;

#ifdef SMPPCM_DEBUG
	#include "..\smppcmd\smppcmd.c"
	#define SMPPCMD_VblIn				smppcmd_VblIn
	#define SMPPCMD_Init				smppcmd_Init
	#define SMPPCMD_MON_Reset			smppcmd_MON_Reset
	#define SMPPCMD_PCM_Task			smppcmd_PCM_Task
#else
	#define SMPPCMD_VblIn()				
	#define SMPPCMD_Init(a)				
	#define SMPPCMD_MON_Reset()			
	#define SMPPCMD_PCM_Task			PCM_Task
	#define VTV_Printf(a)				
	#define VTV_PRINTF(a)				
	#define _VTV_Printf(a)				
	#define _VTV_PRINTF(a)				
#endif


/*--------------------------- 《関数宣言》 ---------------------------*/

void vblInit(void);
void smpVblIn(void);
void smpVblOut(void);
void stmInit(void);
void stmGrpOpen(void);


/*---------------------------- 《定数》 ----------------------------*/

/* ウェーブＲＡＭの転送アドレスとサンプル数 */
#define	PCM_ADDR	((void*)0x25a20000)
#define	PCM_SIZE	(4096L*2)				/* 2.. */

/* リングバッファのサイズ */
#define	RING_BUF_SIZE	(2048L*10)			/* 10.. */

/* データの転送方式（ＣＤブロック→リングバッファ） */
#define TR_MODE_CD		PCM_TRMODE_CPU		/* ソフトウェア転送 			*/
#if 0
#define TR_MODE_CD		PCM_TRMODE_SDMA		/* ＤＭＡサイクルスチール 		*/
#define TR_MODE_CD		PCM_TRMODE_SCU		/* ＳＣＵのＤＭＡ				*/
#endif

/*----------------------- 《グローバル変数》 -----------------------*/

/* ワークバッファ */
PcmWork g_movie_work;

/* リングバッファ */
Uint32 g_movie_buf[RING_BUF_SIZE / sizeof(Uint32)];

/*---------------------------- 《関数》 ----------------------------*/


void errGfsFunc(void *obj, Sint32 ec)
{
/*	VTV_PRINTF((VTV_s, "S:ErrGfs %X %X\n", obj, ec)); */
}

void errStmFunc(void *obj, Sint32 ec)
{
/*	VTV_PRINTF((VTV_s, "S:ErrStm %X %X\n", obj, ec)); */
}

void errPcmFunc(void *obj, Sint32 ec)
{
	/* VTV_PRINTF((VTV_s, "S:ErrPcm %X %X\n", obj, ec));*/
}


/*====================== Ｖブランクの処理 ===========================*/

void vblInit(void) 	/* Ｖブランクの設定 */
{
    set_imask(0xf);
	INT_ChgMsk(INT_MSK_NULL,INT_MSK_VBLK_IN | INT_MSK_VBLK_OUT);
	INT_SetScuFunc(INT_SCU_VBLK_IN, smpVblIn);
	INT_SetScuFunc(INT_SCU_VBLK_OUT, smpVblOut);
	INT_ChgMsk(INT_MSK_VBLK_IN | INT_MSK_VBLK_OUT,INT_MSK_NULL);
    set_imask(0);
}

void smpVblIn(void)
{
	if(PcmInit){
		/* ＰＣＭライブラリの VblIn ルーチンをコール */
		PCM_VblIn();
	}
	if(SclInit){
		/* グラフィックライブラリを使用する為には実行しなければならない */
		SCL_VblankStart();
	}
}

void smpVblOut(void)
{
	if(SclInit){
		/* グラフィックライブラリを使用する為には実行しなければならない */
		SCL_VblankEnd();
	}
}


/*====================== サウンドの処理 ===========================*/
#define SDDRVS_TSK_SIZE			(26900)
#define BOOTSND_MAP_SIZE		(100)
Sint32 sddrvs_tsk[ (SDDRVS_TSK_SIZE + 3) / 4];
Sint32 bootsnd_map[ (BOOTSND_MAP_SIZE + 3) / 4];

Sint32 fileLoad(Sint8 *name, void *addr, Sint32 bsize)
{
	Sint32 		fid, i;

	for (i = 0; i < 10; i++) {
		fid = GFS_NameToId(name);
		if (fid >= 0) {
			GFS_Load(fid, 0, addr, bsize);
			return 0;
		}
	}
	return -1;
}

void sndInit(void)
{
	SndIniDt 	snd_init;

	if (fileLoad("SDDRVS.TSK", (void *)sddrvs_tsk, SDDRVS_TSK_SIZE)) {
		return;
	}
	if (fileLoad("BOOTSND.MAP", (void *)bootsnd_map, BOOTSND_MAP_SIZE)) {
		return;
	}
	SND_INI_PRG_ADR(snd_init) 	= (Uint16 *)sddrvs_tsk;
	SND_INI_PRG_SZ(snd_init) 	= (Uint16 )SDDRVS_TSK_SIZE;
	SND_INI_ARA_ADR(snd_init) 	= (Uint16 *)bootsnd_map;
	SND_INI_ARA_SZ(snd_init) 	= (Uint16)BOOTSND_MAP_SIZE;
	*(Uint16 *)0x25a0a000 = 0xffff;
	SND_Init(&snd_init);
	SND_ChgMap(0);
}


/*====================== ファイルの処理 ===========================*/

#define OPEN_MAX	5		/* 同時に開くファイルの最大数 */
#define GRP_MAX		1		/* 同時に開くストリームグループの最大数 */
#define MAX_DIR		100		/* ルートディレクトリにあるファイルの最大数 */

/* ＧＦＳの作業領域 */
GfsDirTbl	dir_tbl;	/* ディレクトリ情報管理領域 */
GfsDirName dir_name[MAX_DIR]; /* ファイル名を含んだ情報 */

#if 0
GfsMng g_gfs_work;
Uint8 g_gfs_work2[GFS_WORK_SIZE(OPEN_MAX)-sizeof(GfsMng)];
#else
Uint32 gfs_work[GFS_WORK_SIZE4(OPEN_MAX)];
#endif

void fileInit(void)
{
	Sint32 file_num;

	/* エラー関数の設定 */
	GFS_SetErrFunc(errGfsFunc, NULL);

	/* GFSの初期化 */
	GFS_DIRTBL_TYPE(&dir_tbl) = GFS_DIR_NAME;
	GFS_DIRTBL_NDIR(&dir_tbl) = MAX_DIR;
	GFS_DIRTBL_DIRNAME(&dir_tbl) = dir_name;

    /* ファイルシステムの初期化 */
	do{
	    file_num = GFS_Init(OPEN_MAX, &gfs_work, &dir_tbl);
	}while(file_num < 0);  /* 本来は有限回で打ち切ること */
}


StmGrpHn grp_hd;  /* ストリームグループのＩＤ */
Uint32   stm_work[STM_WORK_SIZE4(GRP_MAX, OPEN_MAX)];

void stmInit(void)
{
	/* ストリームシステムの初期化 */
	STM_Init(GRP_MAX, OPEN_MAX, (void *)stm_work);

	/* エラー関数の設定 */
	STM_SetErrFunc(errStmFunc, NULL);
}

void stmGrpOpen(void)
{
	/* ストリームグループのオープン */
	grp_hd = STM_OpenGrp();
	if (grp_hd == NULL) {
		return;
	}
	if(STM_SetLoop(grp_hd, STM_LOOP_DFL, STM_LOOP_ENDLESS) == FALSE)
	/* エラー処理 */;
	if(STM_SetExecGrp(grp_hd) == FALSE)
	/* エラー処理 */;
}

StmHn stmOpen(Sint8 *fname)
{
    Sint32 fid;
	StmKey key;

    /* ファイル名からファイル識別子を求める */
    fid = GFS_NameToId(fname);
    if( fid < 0 )
	STM_KEY_FN(&key) = STM_KEY_CN(&key) = STM_KEY_SMMSK(&key) = 
		STM_KEY_SMVAL(&key) = STM_KEY_CIMSK(&key) = STM_KEY_CIVAL(&key) =
		STM_KEY_NONE;
	return STM_OpenFid(grp_hd, fid, &key, STM_LOOP_NOREAD);
}

void stmClose(StmHn fp)
{
	STM_Close(fp);
}

void main(void)
{
	PcmHn 			pcm;
	StmHn			stm;
	PcmCreatePara	para;
	Uint32			restart;
#ifdef DEBUG
	int	i;
	for(i=0;i<0x4000;i++){
		err_status[i] = 0;
	}
#endif

	fileInit();		/* ファイルシステム初期化  */
	sndInit();  	/* サウンドの設定 */

	stmInit();	/* ストリームシステムの初期化 */
	stmGrpOpen();

	/* ＰＣＭライブラリの初期化 */
	PCM_Init();
	PcmInit = 1;

	/* ＡＤＰＣＭ使用宣言 (ADPCM伸張ライブラリがリンクされます) */
#if 0
	PCM_DeclareUseAdpcm();
#endif
	/* ＰＣＭライブラリのエラーハンドルの設定 */
	PCM_SetErrFunc(errPcmFunc, NULL);

	/* Ｖブランクの設定 */
	SCL_Vdp2Init();
	SclInit = 1;
	vblInit();

	restart = 1;

	while(1) {    /*  main loop  */
		if (restart) {
			/* ストリームオープン */
			if ((stm = stmOpen(filename)) == NULL) {
				return;
			}

			/* 取り出し領域のリセット
			 *	ストリームシステムの仕様が...
			 *	最初の転送を STM_ExecServer で行うなら、これは不要。
			*/
			STM_ResetTrBuf(stm);

			/* ハンドルの作成 */
			PCM_PARA_WORK(&para) = (struct PcmWork *)&g_movie_work;
			PCM_PARA_RING_ADDR(&para) = (Sint8 *)g_movie_buf;
			PCM_PARA_RING_SIZE(&para) = RING_BUF_SIZE;
			PCM_PARA_PCM_ADDR(&para) = PCM_ADDR;
			PCM_PARA_PCM_SIZE(&para) = PCM_SIZE;
			pcm = PCM_CreateStmHandle(&para, stm);
			if (pcm == NULL) {
				return;
			}

			/* データの転送方式の設定（ＣＤブロック→リングバッファ） */
			/* PCM_SetTrModeCd(pcm, TR_MODE_CD); */

			/* 開始 */
			PCM_Start(pcm);

			restart = 0;
		}

		/* サーバの実行 */
		STM_ExecServer();

		/* 再生タスク */
		PCM_Task(pcm);

		/* ムービの終了判定 */
		if (PCM_GetPlayStatus(pcm) == PCM_STAT_PLAY_END) {

			/* ハンドルの消去 */
			PCM_DestroyStmHandle(pcm);

			/* ストリームのクローズ*/
			stmClose(stm);

			restart = 1;
		}
	}  /* while(1)  main loop end */
}
