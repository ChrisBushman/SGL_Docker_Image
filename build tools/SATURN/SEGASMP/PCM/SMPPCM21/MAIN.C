 ...
 *
 *--------------------------------------------------------------------------*
 * Update	:1996-07-05	SaturnPCM ファイルをストリームシステムにより再生
 *
 ****************************************************************************/
...
#ifdef	_DEBUG_XPCM_
#if 0
	static char filename[] = "SAMPLE1.SAP";
#else
	static char filename[] = "RAIN.SAP";
#endif
#else
	static char filename[] = "SAMPLE1.AIF";
#endif

/*  SaturnPCM ファイル再生用拡張ワークエリア  */
static	Sint8	blkbuf[PCM_SAP_XWORK_SIZE];
...
#else
#define SDDRVS_TSK_SIZE			(26620)
#define BOOTSND_MAP_SIZE		(80)
#endif
...
#ifdef	_DEBUG_XPCM_
			/*  ◇ Saturn PCM ファイル  */
			/*  リングバッファは不要。代わりに作業領域を使う  */
			PCM_PARA_WORK(&para) = (struct PcmWork *)&g_movie_work;
			PCM_PARA_XWORK_ADDR(&para) = blkbuf;
