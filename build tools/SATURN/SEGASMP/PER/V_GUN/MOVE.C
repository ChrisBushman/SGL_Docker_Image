...
static trigger_t	GetTriggerGun( void ){
	...
	Uint8	portbit = ( 0x01 << V_GUN_PORT );
	Uint8	data;
	
	SMPC_IOSEL = portbit;
	SMPC_DDR( V_GUN_PORT ) = 0;
#if 0
	SMPC_PDR( V_GUN_PORT ) = ~0;
#else     /* 1997-09-01  by A.H    PDRは、7bitレジスタ */
	SMPC_PDR( V_GUN_PORT ) = 0x7F;
#endif
	SMPC_EXLE  = portbit;

	data = SMPC_PDR( V_GUN_PORT );
...
void	move_cursor( const SysPort	*port, int	*x, int	*y ){
	...
	if( mid == MID_GUN ){
		if( tvstat & VDP2_TVSTAT_EXLTFG ){  /* ラッチ有り */
			/*
			**	バーチャガンは、ブレがものすごいので、
			**	こうでもしないと使いものにならない。
			*/
