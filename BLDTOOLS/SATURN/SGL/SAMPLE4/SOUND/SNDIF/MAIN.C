...
void init_sound(){
    ...
    /*
       Sound Interface library don't set sound transfer end flag.
       So, these process execute by user ownself
    */
    *( Uint8 * )0x25a00504 |= 0x80;
    *( Uint8 * )0x25a0050c |= 0x80;
    *( Uint8 * )0x25a00514 |= 0x80;
    *( Uint8 * )0x25a0051c |= 0x80;
    *( Uint8 * )0x25a00524 |= 0x80;
    *( Uint8 * )0x25a0052c |= 0x80;
    *( Uint8 * )0x25a00534 |= 0x80;
    *( Uint8 * )0x25a0053c |= 0x80;
}
...
void sound_test(EVENT *evptr){
	...
	if(smptr->seq[0].stat & _Play){
	    ...
	    slBGMOn((smptr->banknum << 8) + smptr->songnum,0,127,25) ;
