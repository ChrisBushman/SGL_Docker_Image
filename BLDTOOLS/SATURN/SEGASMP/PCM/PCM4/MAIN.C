 ...
 *	ファイル名は FILENAME に定義する。
 *
 ****************************************************************************/
#if 1
#define FILENAME	"SAMPLE.AIF"
#else
#define FILENAME	"A00.AIF"
#endif
...
#include "sega_snd.h"
#include "sega_pcm.h"
#if 0   /* 1997-09-03  A.H(SOJ)  外部リンケージ方式に変更 */
#include "sddrvs.dat"  /* SOUND DRIVER for SAMPLE */
#else
...
static void sndInit(void)
{
	...
	int sound_map[] = {0x0001,0x0000,0x0001,0x4000,
					...
					0xffff };
	static int errChk;
#if 0   /*  del by A.H  サウンドドライバはインクルードファイルで提供 */
  ...
  /*  SOUND INITIALIZE */
 /* 1997-09-03 A.H(SOJ)  サウンドドライバ 外部リンケージ */
    ...
    SND_Init(&snd_init); /* Initialize the sound system & Start SoundDriver */

    for(errChk = 0;SND_ChgMap(0) == SND_RET_NSET;)
    	if (errChk++ > 512) SYS_EXECDMP();
    for(errChk = 0;SND_SetTlVl(14) == SND_RET_NSET;)
		if (errChk++ > 512) SYS_EXECDMP();
#endif
}
