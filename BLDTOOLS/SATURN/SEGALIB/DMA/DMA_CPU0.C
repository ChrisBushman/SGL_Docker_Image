 ...
 *      1996-10-03  K.K Ver.1.05
 *  MOD HISTORY:
 *      '95-11-14 DRDCR0/DRCR1のDREQ指定禁止への対応 
 *      '96-09-25 1byteカナコード殲滅作戦
 *-----------------------------------------------------------------------------
 */
...
#define R_CHCR_AM    (1 << DMA_CHCR_AM)   /* アクノリッジ/トランスファモードビット          */
...
#define R_CHCR_DM    (3 << DMA_CHCR_DM)   /* ディスティネーションアドレスモードビット        */
/******** DMAオペレーションレジスタ ******************************************/
...
void DMA_CpuSetPrm(DmaCpuPrm *prm, Uint32 ch)
{
    ...
    work_reg &= ~R_CHCR_TB;                 /* トランスファバスモードセット  */
                                            /* (サイクルスチールモード固定)  */
    work_reg &= ~R_CHCR_TA;                 /* トランスファアドレスモードビットセット　　　　*/
                                            /* (デュアルアドレスモード固定)  */
    ...
    if((prm->msk & DMA_CPU_M_TE) == DMA_CPU_M_TE){
        work_reg &= ~R_CHCR_TE;             /* トランスファエンドフラグのクリア           */
    }
...
void DMA_CpuGetComStatus(DmaCpuComStatus *status)
{
    ...
    work_reg = DMA_PEEK_L(REG_DMAOR);
                                    /* DMAオペレーションレジスタの読み込み   */
    status->ae = work_reg & R_DMAOR_AE; /* アドレスモードエラーフラグの取得            */
...
DmaCpuStatus DMA_CpuGetStatus(Uint32 ch)
{
    ...
    work_reg = DMA_PEEK_L(REG_CHCR + OFSET0_REG * ch);
                                        /* DMAチャネルコントロールレジスタの読み込み      */
    return(work_reg & R_CHCR_TE);       /* トランスファエンドフラグの取得              */
}
