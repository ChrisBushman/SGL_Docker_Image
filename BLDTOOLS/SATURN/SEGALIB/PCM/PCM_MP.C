...
void	pcm_MeGetRingWrite_aiff(PcmHn hn, 
	Sint8 **ring_write_addr, Sint32 *write_size, Sint32 *total_write_size);
void	pcm_MeRenewRingWrite_aiff(PcmHn hn, Sint32 write_size);
void	pcm_MeTask_aiff(PcmHn hn);

STATIC Sint32 PCM_MeHeaderProcess(PcmHn hn);
...
/* バージョン文字列 */
const Sint8 pcm_version[PCM_VERSION_SIZ] = PCM_VERSION_STR;
...
/* エラー管理変数 */
PcmErrCode pcm_err_code;				/* エラーコード 		*/
...
/*  MP 実行関数テーブル  */
static	PcmMeExecFunc	pcmme_fntbl_aiff = {
	pcm_MeGetRingWrite_aiff,
	pcm_MeRenewRingWrite_aiff,
	pcm_MeTask_aiff
};

static	PcmMeExecFunc	pcmme_fntbl;
/*  1996.07.08 Y.H  ( end )  */
...
void PCM_MeReset(PcmHn hn)
{
	...
	/*  拡張ステータス  */
	PCMXLIB_STAT_XSTAT(st)	= PCMXLIB_XSTAT_INIT;
	PCMXLIB_STAT_XPARITY(st)	= 0;
	PCMXLIB_STAT_XLRDONE(st)	= 0;
	PCMXLIB_STAT_XBLK_FLAG(st)	= 0;
	PCMXLIB_STAT_XBLK_ROFST(st)	= 0;
	PCMXLIB_STAT_XRING_WADDR(st) = (Sint8 *)st->ring_end_addr;
	PCMXLIB_STAT_XRING_RADDR(st) = (Sint8 *)st->ring_end_addr;
	PCMXLIB_STAT_XBLK_WOFST(st)	= 0;
	PCMXLIB_STAT_XPLAY_ON(st)	= 0;
	PCMXLIB_STAT_XLOAD_ON(st)	= 0;
...
PcmHn PCM_MeCreate(PcmWork *work, PcmPara *para)
{
	...
	if (pcm_sap_flag != OFF) {
		/*  SaturnPCM ファイル  */
		/*  作業領域  */
		blk_adr  = para->ring_addr;
		blk_size = para->ring_size;

		/*  ヘッダ解析時に修正するがとりあえず代入する  */
		para->ring_addr = para->pcm_addr;
		para->ring_size = para->pcm_size;
	}
#if		0		/*  1997-03-10  */
	else {
		/*  AIFF ファイル  */
		/*  リングバッファサイズ ２ｋの倍数  */
		if (para->ring_size & 0x000007ff) {
	...
	*(PcmWork **)hn = work;
	...
	if (pcm_sap_flag != OFF) {
		/*  SaturnPCM ファイル  */
		st->info.file_type = PCM_FILE_TYPE_SAP;
		st->info.data_type = PCM_DATA_TYPE_LLLRRR;
		if (blk_size < PCM_SAP_XWORK_SIZE) {
			/*  ブロックバッファサイズが小さい  */
			return (NULL);
		}
		PCMXLIB_STAT_XBLK_ADDR(st) = blk_adr;
		PCMXLIB_STAT_XBLK_SIZE(st) = blk_size;
		PCMXLIB_STAT_XBLK_FLAG(st) = TRUE;

		/*  再生開始トリガサンプルの設定 [sample/1ch]  */
		PCM_SetStartTrgSample(hn, PCM_SAP_DFLT_SAMPLE_START_TRG);

		/*  再生停止トリガサンプルの設定 [sample/1ch]  */
		PCM_SetStopTrgSample(hn, PCM_SAP_DFLT_SAMPLE_STOP_TRG);

		/* １回のタスクで処理する量の上限の設定 */
		PCM_MeSet1TaskSample(hn, PCM_SAP_DFLT_SAMPLE_1TASK);
	}
	else {
		/*  AIFF ファイル  */
		st->info.file_type = PCM_FILE_TYPE_AIFF;
		st->info.data_type = PCM_DATA_TYPE_LRLRLR;
		PCMXLIB_STAT_XBLK_ADDR(st) = (Sint8 *)NULL;
		PCMXLIB_STAT_XBLK_SIZE(st) = 0;
		PCMXLIB_STAT_XBLK_FLAG(st) = FALSE;
	...
	/* オーディオ処理関数ポインタの設定 */
	st->audio_process_fp = pcm_AudioMix;

	/*  PCMデータ転送に関する情報 (RingBuf -> PCMbuf)	*/
	st->copy_mode_pcm = PCM_TRMODE_CPU;	
...
void	PCM_MeGetRingWrite(PcmHn hn, 
	Sint8 **ring_write_addr, Sint32 *write_size, Sint32 *total_write_size)
{
	(PCMME_GET_RINGWR_FN(&pcmme_fntbl))(
		hn, ring_write_addr, write_size, total_write_size);
...
{
	(PCMME_RENEW_RINGWR_FN(&pcmme_fntbl))(hn, write_size);
 ...
 *	読み取り位置更新（ステレオＬ）
 *------------------------------------------------------*/
void pcm_RenewRingRead2(PcmHn hn, Sint32 read_size)
{
	PcmWork		*work = *(PcmWork **)hn;
	PcmPara		*para 	= &work->para;
	PcmStatus	*st = &work->status;

	PCMXLIB_STAT_XBLK_ROFST(st)  += read_size;
	PCMXLIB_STAT_XRING_RADDR(st) += read_size;
	if (PCMXLIB_STAT_XRING_RADDR(st) >=
				(st->ring_end_addr + para->ring_size)) {
		PCMXLIB_STAT_XRING_RADDR(st) -= para->ring_size;
	}
...
【戻り値】
	なし
【備　考】
・SaturnPCM（SAP）の一時停止処理はある条件を満足しないと機能しない。
　一時停止処理完了は、st->play の値を参照することでわかる。
...
STATIC void pcm_PauseOn(PcmHn hn)
{
	...
	*/
	...
	if (pcm_sap_flag != OFF) {
		...
		/*  読込みポインタを更新する  */
		if (PCMXLIB_STAT_XLOAD_ON(st) == ON) {
			(*st->audio_process_fp)(hn);
		}

		/*  セクタバウダリにするため暫く待つ  */
		for (;;) {
			cnt = pcm_GetTimePcm(hn);
			if ((cnt % PCMLIB_SECT_SIZE) == 0) {
				break;
			}
		}
	}
	...
	if (pcm_sap_flag != OFF) {
		/*  SAP  */
		/*  ポインタ調整  */
		st->ring_write_addr -= offset_rotate;
		if (st->ring_write_addr < para->ring_addr) {
			st->ring_write_addr += para->ring_size;
		}
		st->ring_read_addr = st->ring_write_addr;

		if (PCM_IS_STEREO(st) != 0) {
			PCMXLIB_STAT_XRING_WADDR(st) -= offset_rotate;
			if (PCMXLIB_STAT_XRING_WADDR(st) < st->ring_end_addr) {
				PCMXLIB_STAT_XRING_WADDR(st) += para->ring_size;
			}
			PCMXLIB_STAT_XRING_RADDR(st) = PCMXLIB_STAT_XRING_WADDR(st);
		}
	}
	/*  1996.07.04 Y.H  ( end )  */
...
STATIC	Sint32	PCM_MeHeaderProcess(PcmHn hn)
{
	...
	if (st->ring_write_offset < 4) {
		/*  まだデータ供給されていない								*/
		/*  上位のデータ供給関数でリングバッファ変数が更新される	*/
		return -1;
	}

	/*  PCM_FILE_TYPE：AIFF, AIFC ファイル  */
...
void	PCM_MeStartTimer(PcmHn hn)
{
	...
	*/
	...
	if (PCMXLIB_STAT_XBLK_FLAG(st) != 0) {
		/*  SAP  */
		PCMXLIB_STAT_XPLAY_ON(st) = OFF;
...
【機　能】
	オーディオデータの条件比較
	適合条件：
		・PCMストリーム再生番号(サウンドドライバ用)が等しい
		・PCMバッファ(アドレス、サイズ)が等しい
		・チャネル数(ステレオ、モノラル)が等しい
		・サンプリングビット数が等しい
		・サンプリング周波数が等しい
【マクロ評価値】
	1			：オーディオデータの条件が適合しない。
...
Sint32 PCM_IsRingEmpty(PcmHn hn)
{
	...
	if ( (st->ring_write_offset == st->ring_read_offset) && 
			(PCMXLIB_STAT_XBLK_WOFST(st) == PCMXLIB_STAT_XBLK_ROFST(st)) ) {
		return 1;
	}
