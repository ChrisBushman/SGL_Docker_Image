 ...
 *	Updated by A.H on 1996-10-17 Ver.1.22
 * 			NBG0とNBG1で同時にラインスクロールを使用できない不具合を修正
 *------------------------------------------------------------------------
 */
 ...
 * CAVEATS
 * 
 * Ver 1.22 NBG0とNBG1で同時にラインスクロールを使用できない不具合を修正
 *------------------------------------------------------------------------
 */
void SCL_SetLineParam(SclLineparam *lp)
{
	...
	Scl_n_reg.celladdr = (lp->cell_addr / 2) & 0x0007ffff;;
	if(SclCurSclNum == SCL_NBG1) {
		temp = (temp <<8);
		Scl_n_reg.lineaddr[1] = (lp->line_addr / 2) & 0x0007ffff;;
		Scl_n_reg.linecontrl &= 0x00ff;
		Scl_n_reg.linecontrl |= temp;
	}else{   /*  SCL_NBG0  */
		Scl_n_reg.lineaddr[0] = (lp->line_addr / 2) & 0x0007ffff;;
		Scl_n_reg.linecontrl &= 0xff00;
		Scl_n_reg.linecontrl |= temp;
	}
 ...
 */
	if(lp->interval == 0)  step = 1;
	if(lp->interval == 1)  step = 2;
	if(lp->interval == 2)  step = 4;
	if(lp->interval == 3)  step = 8;
	k = 0;
